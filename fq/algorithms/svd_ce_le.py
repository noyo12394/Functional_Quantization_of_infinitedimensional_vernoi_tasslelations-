...
class SVDCELE(FQAlgorithm):
    ...
    def step(self, Xr: np.ndarray, seed: int) -> dict:
        p = min(32, Xr.shape[1] - 1, Xr.shape[0] - 1)
        t0 = time.perf_counter()
        Z = TruncatedSVD(n_components=max(1, p), random_state=seed).fit_transform(Xr)
        labels = KMeans(n_clusters=self.K, init="k-means++", n_init=1, max_iter=self.max_iter, tol=self.tol, algorithm="elkan", random_state=seed).fit_predict(Z)
        init = centroids_from_labels(Xr, labels, self.K, np.random.default_rng(seed))
        km = KMeans(n_clusters=self.K, init=init, n_init=1, max_iter=2, tol=self.tol, algorithm="lloyd", random_state=seed)
        km.fit(Xr)
