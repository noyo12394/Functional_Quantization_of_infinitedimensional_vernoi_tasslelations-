...
class RPANNFQ(FQAlgorithm):
    ...
    def step(self, Xr: np.ndarray, seed: int) -> dict:
        ...
        Z = rp.fit_transform(Xr)
        centers0 = _sample_centers(Xr, self.K, seed)
        C = rp.transform(centers0)
        return _one_pass(Xr, seed, self.K, projected=Z, center_projected=C)
