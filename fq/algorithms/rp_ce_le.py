...
class RPCELE(FQAlgorithm):
    ...
    def step(self, Xr: np.ndarray, seed: int) -> dict:
        d = min(64, Xr.shape[1], max(2, Xr.shape[0] - 1))
        t0 = time.perf_counter()
        Z = GaussianRandomProjection(n_components=d, random_state=seed).fit_transform(Xr)
        labels = KMeans(n_clusters=self.K, init="k-means++", n_init=1, max_iter=self.max_iter, tol=self.tol, algorithm="lloyd", random_state=seed).fit_predict(Z)
        init = centroids_from_labels(Xr, labels, self.K, np.random.default_rng(seed))
        km = KMeans(n_clusters=self.K, init=init, n_init=1, max_iter=self.max_iter, tol=self.tol, algorithm="elkan", random_state=seed)
        km.fit(Xr)
        return dict(time=time.perf_counter() - t0, sse=float(km.inertia_), iter=int(km.n_iter_), centers=km.cluster_centers_.copy())
