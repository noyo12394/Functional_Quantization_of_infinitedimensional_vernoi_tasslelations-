...
class HK(FQAlgorithm):
    name = "HK"
    heavy = True

    def step(self, Xr: np.ndarray, seed: int) -> dict:
        ag = AgglomerativeClustering(n_clusters=self.K, linkage="ward")
        t0 = time.perf_counter()
        labels = ag.fit_predict(Xr)
        init = centroids_from_labels(Xr, labels, self.K, np.random.default_rng(seed))
        km = KMeans(n_clusters=self.K, init=init, n_init=1, max_iter=self.max_iter, tol=self.tol, algorithm="lloyd", random_state=seed)
        km.fit(Xr)
