...
class CEMB(FQAlgorithm):
    ...
    def step(self, Xr: np.ndarray, seed: int) -> dict:
        mb = MiniBatchKMeans(n_clusters=self.K, init="k-means++", n_init=1, max_iter=self.max_iter, batch_size=min(512, Xr.shape[0]), random_state=seed)
        t0 = time.perf_counter(); mb.fit(Xr)
        return dict(time=time.perf_counter() - t0, sse=float(mb.inertia_), iter=int(mb.n_iter_), centers=mb.cluster_centers_.copy())
