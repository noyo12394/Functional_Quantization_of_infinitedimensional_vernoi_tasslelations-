...
class MRFQ(FQAlgorithm):
    name = "MR-FQ"

    def step(self, Xr: np.ndarray, seed: int) -> dict:
        coarse = Xr[:, ::max(1, Xr.shape[1] // min(128, Xr.shape[1]))]
        km0 = KMeans(n_clusters=self.K, init="k-means++", n_init=1, max_iter=max(2, self.max_iter // 2), tol=self.tol, random_state=seed)
        t0 = time.perf_counter(); labels = km0.fit_predict(coarse)
        init = centroids_from_labels(Xr, labels, self.K, np.random.default_rng(seed))
        km = KMeans(n_clusters=self.K, init=init, n_init=1, max_iter=min(5, self.max_iter), tol=self.tol, algorithm="lloyd", random_state=seed)
        km.fit(Xr)
        return dict(time=time.perf_counter() - t0, sse=float(km.inertia_), iter=int(km0.n_iter_ + km.n_iter_), centers=km.cluster_centers_.copy())
